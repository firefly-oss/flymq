...
/*
Package health provides health check endpoints for FlyMQ.
...
func ClusterCheck(minNodes int, getNodeCount func() int) Check {
	return func() CheckResult {
		count := getNodeCount()
		if count < minNodes {
			return CheckResult{
				Status:  StatusDegraded,
				Message: "insufficient cluster nodes",
			}
		}
		return CheckResult{Status: StatusHealthy}
	}
...
func MemoryCheck(maxUsagePercent float64, getUsage func() float64) Check {
	return func() CheckResult {
		usage := getUsage()
...
// DiskCheck creates a disk usage health check.
func DiskCheck(maxUsagePercent float64, getUsage func() float64) Check {
	return func() CheckResult {
		usage := getUsage()
		if usage > maxUsagePercent {
			return CheckResult{
