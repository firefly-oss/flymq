...
/*
Package cluster provides mDNS/DNS-SD service discovery for FlyMQ nodes.

OVERVIEW:
=========
FlyMQ uses mDNS (multicast DNS) for automatic node discovery on local networks.
This enables zero-configuration cluster setup where nodes can find each other
without manual peer configuration.

SERVICE TYPE:
=============
FlyMQ advertises itself as: _flymq._tcp.local.

Each node publishes:
- Instance name: <node-id>._flymq._tcp.local.
- Port: cluster port (default 9093)
- TXT records: node_id, cluster_id, client_port, version
...
const (
	// ServiceType is the mDNS service type for FlyMQ
	ServiceType = "_flymq._tcp"
...
type DiscoveredNode struct {
	...
	ClientAddr  string // host:port for client connections
	Version     string
...
type DiscoveryConfig struct {
	NodeID       string
	ClusterID    string
	ClusterAddr  string // Address to advertise for cluster traffic
	ClientAddr   string // Address to advertise for client traffic
	Version      string
	Enabled      bool
}
...
func (d *DiscoveryService) Start() error {
	if !d.config.Enabled {
		d.logger.Info("Service discovery disabled")
		return nil
	}
	...
	txtRecords := []string{
		...
		fmt.Sprintf("client_addr=%s", d.config.ClientAddr),
	...
	d.logger.Info("Service discovery started",
		"node_id", d.config.NodeID,
		"cluster_addr", d.config.ClusterAddr,
...
func (d *DiscoveryService) Stop() error {
	...
	d.running = false
	d.logger.Info("Service discovery stopped")
	return nil
}

// DiscoverNodes discovers FlyMQ nodes on the network.
